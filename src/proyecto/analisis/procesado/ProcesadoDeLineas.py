...
class ProcesadoDeLineas():
    ...
    def modulo(self,linea):
        """
        Metodo que se encargara de hacer el modulo a unsegmeneto pasado como 
        parametro a nuestra funcion.
        
        @param linea: Segmento a linia sobre la que calcular el modulo.
        @return: Modulo del segmento pasado como parametro. 
        """
        p1,p2=linea 
        x1,y1 = p1
        x2,y2 = p2
    ...
    @classmethod 
    def combina2(self,distancia1,distancia2,diff_orientacion,longcorto,lines,g): 
        """
        Funcion que combina las lineas cercanas producidas por la Transformada probabilistica
        de hough que cumplen ciertos parametros y las anade a un grafo.

        @param distancia1: Distancia que si superan dos rectas no une
        @param distancia2: Angulo que si superan dos rectas no une
        @param diff_orientacion: diferencia maxima de orientacion para evitar uniones innecesarias.
        @param longcorto: longitud maxima que pondremos como prametro para la union.
        @param lines: Lineas producidas al aplicar la transformada
        @param g: Grafo donde anadir los nodos que tengamso que unir
        ...
        for i in range(len(lines)-1):
            for j in range(i+1,len(lines)):
                ...
                d=[]
                d.extend([d1,d2,distancia1,distancia2,longcorto])
                self.combina_2_compro(d,lines,i,j,diff_orientacion,g)
        return g
    
    @classmethod 
    def combina_2_compro(self,d,lines,i,j,diff_orientacion,g):
        """
        Metodo que comprueba si hay que añadirlo al grafo o no.
        
        @param d: Conjunto de variables necesarias para la ejecucion.
        @param lines: Lineas detectadas en bruto por hough.
        @param i: variable i del bucle.
        @param j: variable j del bucle.
        @param diff_orientacion: diferencia de orientacion que si superan 
        dos rectas no une.
        @param g: grafo donde estan las lineas.
        """
        d1,d2,distancia1,distancia2,longcorto=d
        if (d1<d2 and d1<distancia1) or (d1>=d2 and d2<distancia2): 
            angle = self.ang(lines[i],lines[j])
            if angle <= diff_orientacion or self.modulo(lines[i])<longcorto or self.modulo(lines[j])<longcorto:
                g.add_edge(i,j) 
    ...
    def filtra_contenidas(self,lineas,x_min,x_max,y_min,y_max,lon_min):
        """
        Funcion auxiliar que filtra los segmentos contenidos en el cuadrado del area.
        @return: Segmentos que hay qeu pitnar dentro del area seleccioanda.
        """
        lines_pintar=[]
        for i in lineas:
    ...
    def filtra_intersec(self,lineas,cuadr,variables,lon_min):
        """
        Metodo que filtra las lineas que se interseccionan con el cuadrado de 
        area delimitada de la aplicacion sobre las uqe evaluar los segmentos calculados.
        
        @return: Devuelve la lista con las lineas que hay contenidas dentro del cudrado.
        """
        nuevas=[]
    ...
    def prueba_point2(self,existe_punto2,marc,un,i,j,dos):
        """
        Fauncion que prueba si eciste un punto dos.
        return atributos necesarios para la ejecucion de la funcion global.
        """
        x2,y2,add=dos
    ...
    def perp( self,a ) :
        """
        Funcionque calcula la perpendicular de iun punto dado.
        @return: punto b
        """
        b = np.empty_like(a)
        b[0] = -a[1]
    ...
    def seg_intersect(self,segment1, segment2) :
        """
        Metodo que calcula los pntos de interseccion entree dos segmentos.
        @return: Puntos de interseccion en forma de lista.
        """
